...
export default function TransferList({ addRoles, companyId, userId }) {
	...
	useEffect(() => {
		axios
			.get("/api/permission")
			.then((response) => {
				let bits = response.data.permissions.map((p) => p.bit);

				if (userId) {
					axios
						.get(
							`http://localhost:3001/api/company/${companyId}/users/${userId}/permissions`
						)
						.then((userPermissionResponse) => {
							console.log(userPermissionResponse);

							const rightBits = userPermissionResponse.data.permissions.map(
								(p) => p.bit
							);

							rightBits.map((bit) => {
								return (bits = bits.filter((e) => e !== bit));
							});

							console.log(rightBits);
							console.log(bits);
