...
export default class Timeline extends React.Component {
	...
	static defaultProps = {
		...
		timelineMode:
			Timeline.TIMELINE_MODES.SELECT |
			Timeline.TIMELINE_MODES.DRAG |
			Timeline.TIMELINE_MODES.RESIZE,
	...
	constructor(props) {
		...
		this.throttledMouseMoveFunc = _.throttle(
			this.throttledMouseMoveFunc.bind(this),
			20
		);
		this.mouseMoveFunc = this.mouseMoveFunc.bind(this);
		this.getCursor = this.getCursor.bind(this);

		const canSelect = Timeline.isBitSet(
			Timeline.TIMELINE_MODES.SELECT,
			this.props.timelineMode
		);
		const canDrag = Timeline.isBitSet(
			Timeline.TIMELINE_MODES.DRAG,
			this.props.timelineMode
		);
		const canResize = Timeline.isBitSet(
			Timeline.TIMELINE_MODES.RESIZE,
			this.props.timelineMode
		);
	...
	componentWillUnmount() {
		...
		if (this._selectRectangleInteractable)
			this._selectRectangleInteractable.unset();
	...
	componentDidUpdate(prevProps, prevState) {
		...
		const selectionChange = !_.isEqual(
			prevProps.selectedItems,
			selectedItems
		);
		const timelineModeChange = !_.isEqual(
			prevProps.timelineMode,
			timelineMode
		);
	...
	setTimeMap(items, startDate, endDate) {
		...
		_.forEach(itemRows, (visibleItems, row) => {
			...
			if (this.rowItemMap[rowInt] === undefined)
				this.rowItemMap[rowInt] = [];
	...
	changeGroup(item, curRow, newRow) {
		...
		this.rowItemMap[curRow] = this.rowItemMap[curRow].filter(
			(i) => i.key !== item.key
		);
	...
	setUpDragging(canSelect, canDrag, canResize) {
		...
		this._selectRectangleInteractable = interact(
			`.${topDivClassId} .parent-div`
		);

		this._itemInteractable.on("tap", (e) => {
			this._handleItemRowEvent(
				e,
			...
			this._itemInteractable
				...
				.on("dragstart", (e) => {
					...
					_.forEach(animatedItems, (id) => {
						let domItem = this._gridDomNode.querySelector(
							"span[data-item-index='" + id + "'"
						);
						if (domItem) {
							selections.push([
								this.getItem(id).start,
								this.getItem(id).end,
							]);
				...
				.on("dragmove", (e) => {
					...
					let animatedItems =
						this._gridDomNode.querySelectorAll(
							"span[isDragging='True'"
						) || [];

					let dx =
						(parseFloat(target.getAttribute("drag-x")) || 0) + e.dx;
					let dy =
						(parseFloat(target.getAttribute("drag-y")) || 0) + e.dy;
					let selections = [];
					...
					_.forEach(animatedItems, (domItem) => {
						...
						let newPixelOffset =
							pixToInt(domItem.style.left) + snapDx;
						...
						domItem.style.webkitTransform = domItem.style.transform =
							"translate(" + snapDx + "px, " + dy + "px)";
					});
				...
				.on("dragend", (e) => {
					...
					let newPixelOffset =
						...
						(parseFloat(e.target.getAttribute("drag-x")) || 0);
					let newStart = getTimeAtPixel(
						newPixelOffset,
						this.props.startDate,
						this.props.endDate,
					...
					const timeDelta = newStart
						.clone()
						.diff(item.start, "minutes");
		...
		if (canResize) {
			this._itemInteractable
				...
				.on("resizestart", (e) => {
					const selected = this.props.onInteraction(
				...
				.on("resizemove", (e) => {
					...
					let dw =
						e.rect.width -
						Number(e.target.getAttribute("initialWidth"));
					...
					_.forEach(animatedItems, (item) => {
						item.style.width = intToPix(
							Number(item.getAttribute("initialWidth")) +
								snappedDw +
								minimumWidth
						);
						item.style.webkitTransform = item.style.transform =
							"translate(" + snappedDx + "px, 0px)";
					});
				...
				.on("resizeend", (e) => {
					...
					// Update time
					const dx =
						parseFloat(e.target.getAttribute("delta-x")) || 0;
					...
					_.forEach(animatedItems, (domItem) => {
						let startPixelOffset =
							pixToInt(domItem.style.left) + dx;
						...
						if (isStartTimeChange) {
							...
							if (durationChange === null)
								durationChange = item.start.diff(
									newStart,
									"minutes"
								);
							item.start = newStart;
					...
					this.props.onInteraction(
						Timeline.changeTypes.resizeEnd,
						changes,
						items
					);
		...
		if (canSelect) {
			this._selectRectangleInteractable
				...
				.styleCursor(false)
				.on("dragstart", (e) => {
					const nearestRowObject = getNearestRowObject(
						e.clientX,
						e.clientY
					);
					...
					this._selectBox.start(
						e.clientX,
						nearestRowObject.getBoundingClientRect().y
					);
				...
				.on("dragmove", (e) => {
					...
					const currentRowObject = getNearestRowObject(
						clientX,
						clientY
					);
					if (
						currentRowObject !== undefined &&
						startRowObject !== undefined
					) {
						// only run if you can detect the top row
						const startRowNumber = getRowObjectRowNumber(
							startRowObject
						);
						const currentRowNumber = getRowObjectRowNumber(
							currentRowObject
						);
						// const numRows = 1 + Math.abs(startRowNumber - currentRowNumber);
						const rowMarginBorder = getVerticalMarginBorder(
							currentRowObject
						);
						if (startRowNumber <= currentRowNumber) {
							// select box for selection going down
							// get the first selected rows top
							const startTop = Math.ceil(
				...
				.on("dragend", (e) => {
					...
					if (topRowObject !== undefined) {
						...
						const topRowNumber = Number(
							getNearestRowNumber(left, top)
						);
						...
						const bottomRow = Number(
							getNearestRowNumber(
								left + width,
								Math.floor(topRowLoc.top - rowMarginBorder) +
									Math.floor(height - rowMarginBorder)
							)
						);
						...
						let selectedItems = [];
						for (
							let r = Math.min(topRowNumber, bottomRow);
							r <= Math.max(topRowNumber, bottomRow);
							r++
						) {
							selectedItems.push(
								..._.filter(this.rowItemMap[r], (i) => {
									return (
										i.start.isBefore(endTime) &&
										i.end.isAfter(startTime)
									);
								})
							);
						}
						this.props.onInteraction(
							Timeline.changeTypes.itemsSelected,
							selectedItems
						);
					}
				});
	...
	_handleItemRowEvent = (e, itemCallback, rowCallback) => {
		...
		if (
			e.target.hasAttribute("data-item-index") ||
			e.target.parentElement.hasAttribute("data-item-index")
		) {
			let itemKey =
			...
			let snappedClickedTime = timeSnap(
				clickedTime,
				this.props.snapMinutes * 60
			);
	...
	cellRenderer(width) {
		...
		return ({ columnIndex, key, parent, rowIndex, style }) => {
			...
			if (itemCol === columnIndex) {
				...
				const layersInRow = rowLayers.filter(
					(r) => r.rowNumber === rowIndex
				);
				...
				return (
					<div
						...
						onMouseOver={(e) => {
							this.selecting = false;
							return this._handleItemRowEvent(
								e,
								onItemHover,
								null
							);
						...
						onContextMenu={(e) =>
							this._handleItemRowEvent(
								e,
								this.props.onItemContextClick,
								this.props.onRowContextClick
							)
						}
						onDoubleClick={(e) =>
							this._handleItemRowEvent(
	...
	getCursor() {
		...
		return showCursorTime && cursorTime
			? cursorTime.clone().format(cursorTimeFormat)
			: null;
	}
	...
	throttledMouseMoveFunc(e) {
		...
		const leftOffset = document
			.querySelector(`.rct9k-id-${componentId} .parent-div`)
		...
		if (
			...
			this.mouse_snapped_time.unix() !== cursorSnappedTime.unix()
		) {
