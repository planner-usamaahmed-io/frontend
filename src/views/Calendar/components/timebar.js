...
export default class Timebar extends React.Component {
	...
	guessResolution(start, end) {
		...
		//    -> 1h
		if (durationSecs <= 60 * 60)
			this.setState({ resolution: { top: "hour", bottom: "minute" } });
		// 1h -> 3d
		else if (durationSecs <= 24 * 60 * 60 * 3)
			this.setState({ resolution: { top: "day", bottom: "hour" } });
		// 1d -> 30d
		else if (durationSecs <= 30 * 24 * 60 * 60)
			this.setState({ resolution: { top: "month", bottom: "day" } });
		//30d -> 1y
	...
	renderTopBar() {
		...
		return this.renderBar({
			format: this.props.timeFormats.majorLabels[res],
			type: res,
		});
	}
	/**
	...
	renderBottomBar() {
		...
		return this.renderBar({
			format: this.props.timeFormats.minorLabels[res],
			type: res,
		});
	}
	/**
	...
	renderBar(resolution) {
		...
		if (resolution.type === "year") {
			...
			addTimeIncrement(
				moment.duration(0),
				"minutes",
				(currentDt, offst) => currentDt.add(1, "minutes")
			);
		}
	...
	render() {
		...
		if (
			topBarComponent.length > 1 &&
			topBarComponent[1].size > topBarComponent[0].size
		)
			topBarCursorKey = topBarComponent[1].key;
		else if (topBarComponent.length > 0)
			topBarCursorKey = topBarComponent[0].key;

		return (
			<div className="rct9k-timebar">
				<div
					className="rct9k-timebar-group-title"
					style={{ width: this.props.leftOffset }}
				>
					<GroupTitleRenderer />
				</div>
				<div
					className="rct9k-timebar-outer"
					style={{
						width: this.props.width,
						paddingLeft: this.props.leftOffset,
					}}
				>
					<div className="rct9k-timebar-inner rct9k-timebar-inner-top">
						{_.map(topBarComponent, (i) => {
							...
							if (i.isSelected)
								className += " rct9k-timebar-item-selected";
							return (
								<span
									className={className}
									key={i.key}
